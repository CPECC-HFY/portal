  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b bg-background/95 px-4 sm:px-6 backdrop-blur-sm supports-[backdrop-filter]:bg-background/60">
      ...
      <div className="hidden md:flex items-center gap-1.5 text-sm">
        ...
        {crumbs.map((crumb, i) => {
          const raw = crumb.charAt(0).toUpperCase() + crumb.slice(1).replace(/-/g, " ");
          const translatedCrumb = t.has(crumb) ? t(crumb) : raw;
      ...
      <div className="ms-auto flex items-center gap-1">
        ...
        >
          ...
          <span className="text-sm font-normal">{t("searchPlaceholder")}</span>
          <kbd className="pointer-events-none ms-auto hidden h-5 select-none items-center gap-1 rounded-sm border border-muted-foreground/20 bg-muted px-1.5 font-mono text-[10px] font-medium text-foreground/70 opacity-100 sm:flex">
